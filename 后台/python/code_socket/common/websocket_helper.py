...
def sendMessage_only(msg, item):
    ...
    if send_message != None and len(send_message) > 0:
        connectionlist['connection' + str(item)].send(send_message)
            ...
            if self.handshaken == False:  # 如果没有进行握手
                ...
                if self.length_buffer - g_header_length < g_code_length:
                    ...
                    if recv_message == "quit":
                        ...
                        sendMessage_only("%s %s say: %s" % (nowTime, self.remote, self.name + " Logout"), self.index)
                        deleteconnection(str(self.index))
                        self.conn.close()
                        break
                    else:
                        nowTime = time.strftime('%H:%M:%S', time.localtime(time.time()))
                        sendMessage_only("%s %s say: %s" % (nowTime, self.remote, recv_message), self.index)
                    json={}
                    json["aaa"] = "111"
                    json["bbb"] = "222"
                    sendMessage_only(str(json), self.index)
                    g_code_length = 0
                    self.length_buffer = 0
                    self.buffer_utf8 = b""
